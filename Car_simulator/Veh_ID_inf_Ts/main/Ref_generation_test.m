%% Ref Generation
clear ; clc ; close all
Ts = 0.01; Tend = 100; Td = 60; Tf = 0.001;
t_sim = 0:Ts:Tend;
% load ID_inf_low2;

%Acceleration Signal
Brake_str1 = 30; Brake_end1 = 40; Brake_du1 = Brake_end1 - Brake_str1;%(sec)
Brake_str2 = 55; Brake_end2 = 75; Brake_du2 = Brake_end2 - Brake_str2;%(sec)
Brake_str3 = 80; Brake_end3 = 85; Brake_du3 = Brake_end3 - Brake_str3;%(sec)
Brake_str4 = 90; Brake_end4 = 95; Brake_du4 = Brake_end4 - Brake_str4;


Accelcmd = -0.5*(-1+cos(2*pi*t_sim/Td))*0.1;
...
y4 = 0.24*normpdf(t_brake4,0.6,0.6);
...
Steer = 0.3*5/3*pi*sin(t_sim/8);Steer = Steer';
% Steer(5000:5931,:) = 0; 
% Steer(2501:3905,:) = 0; 
% Steer(9431:end,:) = 0; 
...
subplot(2,1,2)
plot(t_sim,Yaw); title('Yaw'); xlabel('Time (s)'); ylabel('Yaw (rad)');
...
save('ID_inf_test.mat','Steer','Accelcmd','t_sim','Yaw','Velocity','XYplot');