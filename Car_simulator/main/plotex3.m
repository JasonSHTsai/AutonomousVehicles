...
title('(a) Steer angle'); xlabel('Time (s)'); ylabel('Steer (deg)');ylim([-500  500]);xlim([160 200]);
...
title('(b) Throttle & Brake'); xlabel('Time (s)'); ylabel('Throttle & Brake');ylim([-1.5 1.5]);xlim([160 200]);
% subplot(3,1,3) 
...
subplot(2,1,2);
plot(t_sim, Y1(:,2)*180/pi); hold on; plot(t_sample,Desired_Heading_angle*180/pi,'.','LineWidth', 0.6);xlim([0 260]);
...
figure();
plot(t_sim,(Near_distance_error*100),'k ');xlim([0 260]);
...
hold on;
plot(XYplot_sim1(:,2),XYplot_sim1(:,1),'b');
plot(XYplot_s(A,2),XYplot_s(A,1),'* k');text(466.8,200.1,'A'); %-268.8,437.6  -472,450 62.75,332.8
plot(XYplot_s(B,2),XYplot_s(B,1),'* k');text(920.6,364.1,'B'); %-485.8,869.1  -1333,650  185,243.2
plot(XYplot_s(end,2),XYplot_s(end,1),'* k');text(1102,1114,'C'); %-1238,1000  -1750,1127  -173,320
title('Paths');xlabel('Y (m)'); ylabel('X (m)');
legend('Desired path', 'Simulator path','location','southwest');

figure();
plot(t_sim, Lambda(:,2) , 'k');
title('Lambda'); ylabel('\lambda_s_i_m_u_2(\itkT)'); xlabel('Time (s)')