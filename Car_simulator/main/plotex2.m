...
title('(a) Steer angle'); xlabel('Time (s)'); ylabel('Steer (deg)');ylim([-500  500]);xlim([40 75]);
...
title('(b) Throttle & Brake'); xlabel('Time (s)'); ylabel('Throttle & Brake');ylim([-1.5 1.5]);xlim([40 75]);
% subplot(3,1,3) 
...
title('(a) Desired velocity vs. Simulator updated velocity'); xlabel('Time (s)');ylabel('Velocity (m/s)');xlim([0 250]);
ylim([0 15]);
legend('Simulator updated velocity','Desired velocity','location','southeast')
subplot(2,1,2);
plot(t_sim, Y1(:,2)*180/pi); hold on; plot(t_sample,Desired_Heading_angle*180/pi,'.','LineWidth', 0.6);xlim([0 250]);
...
xlabel('Time (s)');ylabel('Velocity (m/s)');xlim([0 250]);
legend('Simulator updated velocity','Desired velocity','location','southeast')
...
figure();
plot(t_sim,(Near_distance_error*100),'k ');xlim([0 250]);
...
hold on;
plot(XYplot_sim1(:,2),XYplot_sim1(:,1),'b');
plot(XYplot_s(A,2),XYplot_s(A,1),'* k');text(-437.1,-475.6,'A'); %-268.8,437.6  -472,450 62.75,332.8
plot(XYplot_s(B,2),XYplot_s(B,1),'* k');text(-758.4,-1273,'B'); %-485.8,869.1  -1333,650  185,243.2
plot(XYplot_s(end,2),XYplot_s(end,1),'* k');text(-1173,-1563,'C'); %-1238,1000  -1750,1127  -173,320
title('Paths');xlabel('Y (m)'); ylabel('X (m)');
legend('Desired path', 'Simulator path','location','southwest');
