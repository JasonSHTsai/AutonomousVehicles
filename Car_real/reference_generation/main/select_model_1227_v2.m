function [Acceleration,Angular_acceleration,temp,select_Model] = select_model_1227(Velocity,Heading,Low_Speed,Medium_Speed,High_Speed,Overlap_Maxium,N)
%% define 18 models
% 低速加速(LA)
%   右轉:LAR 1
%   左轉:LAL 2
...
for i=1:length(Angular_acceleration)
    if abs(Angular_acceleration(i,1)) <= 1e-5
        Angular_acceleration(i,1) =0;
    end
%     if abs(Acceleration(i,1)) <= 5e-3
%         Acceleration(i,1) =0;
%     end
end
...
for i = 1:len 
    ...
    if i==1
        ...
        if max_num < ceil(N * Overlap_Maxium / 2)
